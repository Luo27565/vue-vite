<template>
  ...
  <div class="container">
    <div class="captcha" :style="{'background-image':`url(${imgSrc})`}" id="captcha">
      <div class="handle" id="handle"><span @mousedown="mouseDown(true)"></span></div>
    </div>
  </div>
</template>
...
export default {
  name: "Captcha",
  setup: () => {
    const imgSrc = ref('https://images.unsplash.com/photo-1606787366850-de6330128bfc?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2700&q=80;')
    const shouldMove = ref(false)
    const mouseDown = flag => {
      shouldMove.value = flag
    }
    const reset = () => {
...
.captcha::before, .captcha::after {
  ...
  clip-path: inset(
      calc((var(--height) - var(--puzzle-height)) / 2) /* 90px */ var(--puzzle-width) /* 90px */ calc((var(--height) - var(--puzzle-height)) / 2) /* 90px */ calc(var(--width) - var(--puzzle-width) * 2) /* 230px */
  );
